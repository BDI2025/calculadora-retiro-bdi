...
def simular_monte_carlo(
    ...
    """
    Simula n trayectorias. Cada año se sortea un retorno anual uniforme entre min y max.
    Devuelve matriz (n_sim, total_meses+1) con saldos por mes (col 0 = capital inicial).
    """
    ...
    # Sortear retornos anuales por simulación y por año
    retornos_anuales = rng.uniform(min_anual, max_anual, size=(n_simulaciones, anios))
...
def tab_montecarlo(capital_inicial, aporte_mensual, anios, retorno, capitalizacion) -> None:
    st.markdown(f'<div class="section-title">SIMULACIÓN MONTE CARLO</div>', unsafe_allow_html=True)
    st.markdown(
        f'<div style="font-size:12px; color:{BDI_GRIS_SUAVE}; margin-bottom:10px;">'
        f'En lugar de un único retorno fijo, sorteamos un retorno aleatorio por año dentro de un rango. '
        f'Mostramos miles de futuros posibles y los percentiles más relevantes.</div>',
        unsafe_allow_html=True,
    )
    ...
    with col1:
        st.markdown(
            f'<div style="font-size:13px; font-weight:500; color:{BDI_VERDE}; letter-spacing:1px; margin-bottom:6px;">🎲 RANGO DE RENDIMIENTO ANUAL</div>',
            unsafe_allow_html=True,
        )
        preset = st.selectbox(
            "Preset de mercado",
            ["S&P 500 (1928–2024)", "Personalizado"],
            index=0,
        )
        if preset == "S&P 500 (1928–2024)":
            min_default, max_default = -37.0, 38.0
            st.caption("Histórico: peor año -37% (2008), mejor año +38% (1933), promedio ~10%")
        else:
            min_default, max_default = -20.0, 30.0

        min_anual = st.slider("Peor año (%)", -50.0, 0.0, min_default, 1.0) / 100
        max_anual = st.slider("Mejor año (%)", 0.0, 60.0, max_default, 1.0) / 100
    ...
    with st.spinner(f"Ejecutando {n_simulaciones} trayectorias..."):
        trayectorias = simular_monte_carlo(
            capital_inicial, aporte_mensual, anios, min_anual, max_anual, n_simulaciones, seed=seed
        )
    ...
    with col2:
        ...
        with cA:
            st.markdown(
                f'<div class="kpi-card"><div class="kpi-label">PESIMISTA · P10</div>'
                f'<div class="kpi-value" style="color:{BDI_GRIS_SUAVE};">${p10:,.0f}</div>'
                f'<div style="font-size:11px; color:{BDI_GRIS_SUAVE};">10% peores casos</div></div>',
                unsafe_allow_html=True,
            )
        with cB:
            st.markdown(
                f'<div class="kpi-card kpi-card-primary"><div class="kpi-label">ESPERADO · MEDIANA</div>'
                f'<div class="kpi-value">${p50:,.0f}</div>'
                f'<div style="font-size:11px; color:{BDI_LIMA};">50% por arriba / 50% por abajo</div></div>',
                unsafe_allow_html=True,
            )
        with cC:
            st.markdown(
                f'<div class="kpi-card"><div class="kpi-label">OPTIMISTA · P90</div>'
                f'<div class="kpi-value" style="color:{BDI_CYAN};">${p90:,.0f}</div>'
                f'<div style="font-size:11px; color:{BDI_GRIS_SUAVE};">10% mejores casos</div></div>',
                unsafe_allow_html=True,
            )
        ...
        if prob_meta is not None:
            st.markdown(
                f"""
                <div style="background:linear-gradient(90deg, {BDI_LIMA}, {BDI_CYAN}); padding:12px 18px; border-radius:8px; margin-top:12px; display:flex; align-items:center; gap:14px;">
                    <div style="font-family:'Bebas Neue', Impact, sans-serif; font-size:36px; color:{BDI_VERDE}; letter-spacing:1px;">{prob_meta:.0f}%</div>
                    <div style="font-size:14px; color:{BDI_VERDE};">de probabilidad de superar tu meta de <strong>${meta_capital:,.0f}</strong> al año {anios}</div>
                </div>
                """,
                unsafe_allow_html=True,
            )
    ...
    p25_t = np.percentile(trayectorias, 25, axis=0)
    p50_t = np.percentile(trayectorias, 50, axis=0)
    p75_t = np.percentile(trayectorias, 75, axis=0)
    ...
    fig.add_trace(go.Scatter(
        ...
        fill="toself", fillcolor="rgba(181, 230, 29, 0.30)",
        line=dict(color="rgba(0,0,0,0)"),
        name="P10–P90", hoverinfo="skip",
    ))
    fig.add_trace(go.Scatter(
        x=np.concatenate([anios_eje, anios_eje[::-1]]),
        y=np.concatenate([p75_t, p25_t[::-1]]),
        fill="toself", fillcolor="rgba(23, 190, 187, 0.45)",
        line=dict(color="rgba(0,0,0,0)"),
    ...
    fig.add_trace(go.Scatter(
        ...
        name="Mediana",
        hovertemplate="Año %{x:.1f}<br>Mediana: $%{y:,.0f}<extra></extra>",
    ))
    if meta_capital > 0:
        fig.add_hline(
            y=meta_capital, line_dash="dash", line_color=BDI_GRIS_SUAVE,
